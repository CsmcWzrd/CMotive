...
Historical baseline version: 0.2.5-cfrontend-merged

This report records the merge performed before the self-host promotion. For
current results, see `VERIFY_SELFHOST_FRONTEND.md`.
...
The then-active C frontend was reviewed and patched for the merged examples and whole-tree language-file validation. Those fixes are retained in the current self-hosted frontend. They include:
...
- Template placeholder object types such as `Box<I32>` compile cleanly in the frontend scaffold path while concrete template ABI work matures.
