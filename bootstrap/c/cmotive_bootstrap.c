/*
 * CMotive stage-0 bootstrap seed.
 *
 * This isolated C seed exists only to build the first CMotive-hosted
 * frontend.  The active compiler, preprocessor and tool-driver source is
...
#define CMOTIVE_VERSION "0.3.0-selfhost-bootstrap"
...
typedef struct Program { Class *classes; int class_n, class_cap; Func *funcs; int func_n, func_cap; Global *globals; int global_n, global_cap; Field *dyn_fields; int dyn_n, dyn_cap; char *dyn_name; TemplateUse *tuses; int tuse_n, tuse_cap; StrVec native_includes; } Program;
...
static char *unquote_token(const char *s) { size_t n = strlen(s ? s : ""); char *r; if (n >= 2 && ((s[0] == '"' && s[n-1] == '"') || (s[0] == '\'' && s[n-1] == '\''))) { r = (char*)xmalloc(n - 1u); memcpy(r, s + 1, n - 2u); r[n-2] = 0; return r; } return xstrdup(s ? s : ""); }
static void parse_program(Parser *p) { while (p->i < p->n && pk(p,0)->kind != TK_EOF) { skip_eol(p); if (pk(p,0)->kind == TK_EOF) break; if (parser_accept(p,";")) continue; if (streq(pk(p,0)->v,"NativeInclude")) { char *raw, *inc; p->i++; raw=take_value(p); inc=unquote_token(raw); sv_push(&p->prog->native_includes,inc); free(raw); free(inc); parser_accept(p,";"); continue; } if (streq(pk(p,0)->v,"Package") || streq(pk(p,0)->v,"package")) { parse_package(p); continue; } if (streq(pk(p,0)->v,"Plugin")) { p->i++; while (p->i < p->n && pk(p,0)->kind != TK_EOF && pk(p,0)->kind != TK_EOL && !streq(pk(p,0)->v,";")) p->i++; if (streq(pk(p,0)->v,";")) p->i++; continue; } if (streq(pk(p,0)->v,"Class") || streq(pk(p,0)->v,"class")) { parse_class(p); continue; } if (tok_is_decorator(p) || streq(pk(p,0)->v,"Overridable")) { p->i++; continue; } if (streq(pk(p,0)->v,"Dynamic") && streq(pk(p,1)->v,"Struct")) { parse_dynamic_struct(p); continue; } if (p->prog->dyn_name && streq(pk(p,0)->v,p->prog->dyn_name) && streq(pk(p,1)->v,"Expand")) { parse_dynamic_expand(p); continue; } if (streq(pk(p,0)->v,"Template")) { skip_template(p); continue; } if (streq(pk(p,0)->v,"Blend") || streq(pk(p,0)->v,"Union") || streq(pk(p,0)->v,"Enum")) { p->i++; while (p->i < p->n && pk(p,0)->kind != TK_EOF && !streq(pk(p,0)->v,"{")) p->i++; if (streq(pk(p,0)->v,"{")) { int close = block_end(p->t, p->n, p->i); p->i = close + 1; } parser_accept(p,";"); continue; } if (streq(pk(p,0)->v,"Fptr")) { parse_fptr(p); continue; } if (streq(pk(p,0)->v,"Hit")) { char *sender=xstrdup(""), *hid=xstrdup(""); parser_accept(p,"Hit"); if (parser_accept(p,":")) { free(hid); hid=take_value(p); } else if (pk(p,0)->kind == TK_ID && streq(pk(p,1)->v,":")) { free(sender); sender=take_value(p); parser_accept(p,":"); free(hid); hid=take_value(p); } { Func f = parse_func_formal(p,NULL,NULL,NULL,0,0); f.hit_sender=sender; f.hit_id=hid; prog_add_func(p->prog,f); } continue; } if (streq(pk(p,0)->v,"Global") || (pk(p,0)->kind == TK_ID && streq(pk(p,1)->v,":"))) { parse_global(p); continue; } if (streq(pk(p,0)->v,"func")) { Func f = parse_func_keyword(p,NULL); prog_add_func(p->prog,f); continue; }
...
static char *map_builtin_method(const char *type, const char *method) { Str b; sb_init(&b); if (streq(type,"OStream")) { if (streq(method,"Expect")||streq(method,"expect")) sb_add(&b,"CMOStream_Expect"); else if (streq(method,"Write")||streq(method,"write")) sb_add(&b,"CMOStream_Write"); else if (streq(method,"WriteInt")||streq(method,"writeInt")) sb_add(&b,"CMOStream_WriteInt"); else if (streq(method,"Println")||streq(method,"println")) sb_add(&b,"CMOStream_Println"); else if (streq(method,"Flush")||streq(method,"flush")) sb_add(&b,"CMOStream_Flush"); }
 else if (streq(type,"IStream")) { if (streq(method,"Expect")||streq(method,"expect")) sb_add(&b,"CMIStream_Expect"); else if (streq(method,"ReadInt")||streq(method,"readInt")) sb_add(&b,"CMIStream_ReadInt"); else if (streq(method,"ReadString")||streq(method,"readString")) sb_add(&b,"CMIStream_ReadString"); }
...
static int next_statement_token(Tok *t, int n, int i)
{
    while (i < n && t[i].kind == TK_EOL) i++;
    return i;
}
static int statement_extent(Tok *t, int n, int start)
{
    int i = next_statement_token(t, n, start);
    int open, close, body_end, j;
    if (i >= n) return n;
    if (streq(t[i].v, "{")) return block_end(t, n, i) + 1;
    if (streq(t[i].v, "If") || streq(t[i].v, "if")) {
        open = i + 1;
        while (open < n && !streq(t[open].v, "(")) open++;
        close = matching_paren(t, n, open);
        body_end = statement_extent(t, n, close + 1);
        j = next_statement_token(t, n, body_end);
        if (j < n && (streq(t[j].v, "Else") || streq(t[j].v, "else")))
            return statement_extent(t, n, j + 1);
        return body_end;
    }
    if (streq(t[i].v, "For") || streq(t[i].v, "for") ||
        streq(t[i].v, "While") || streq(t[i].v, "while") ||
        streq(t[i].v, "Switch") || streq(t[i].v, "switch")) {
        open = i + 1;
        while (open < n && !streq(t[open].v, "(")) open++;
        close = matching_paren(t, n, open);
        return statement_extent(t, n, close + 1);
    }
    if (streq(t[i].v, "Do") || streq(t[i].v, "do")) {
        body_end = statement_extent(t, n, i + 1);
        j = next_statement_token(t, n, body_end);
        if (j < n && (streq(t[j].v, "While") || streq(t[j].v, "while"))) {
            int semi = stmt_end(t, n, j);
...
static void emit_statement_contents(BodyCtx *bc, Tok *t, int n, int start, int end)
{
    start = next_statement_token(t, n, start);
    if (start >= end || start >= n) return;
    if (streq(t[start].v, "{")) {
        int close = block_end(t, n, start);
        if (close > end) close = end;
        emit_body_range(bc, t + start + 1, close - start - 1);
    } else {
        emit_body_range(bc, t + start, end - start);
    }
}
...
static int init_is_zero(const char *init)
{
    const char *p = init ? init : "";
    size_t n;
    while (*p && isspace((unsigned char)*p)) p++;
...
static void emit_body_range(BodyCtx *bc, Tok *t, int n)
{
    ...
    while (i < n) {
        ...
        if (streq(t[i].v, "If") || streq(t[i].v, "if")) {
            ...
            while (condopen < n && !streq(t[condopen].v, "(")) condopen++;
            condclose = matching_paren(t, n, condopen);
            bodystart = next_statement_token(t, n, condclose + 1);
            bodyend = statement_extent(t, n, bodystart);
            cond = expr_from_tokens(bc->prog, t + condopen + 1, condclose - condopen - 1, &bc->vars, bc->curcls, 0);
            emit_indent(bc->out, bc->indent);
            sb_printf(bc->out, "if (%s) {\n", cond);
            free(cond);
            bc->indent++;
            emit_statement_contents(bc, t, n, bodystart, bodyend);
            bc->indent--;
            emit_indent(bc->out, bc->indent);
            sb_add(bc->out, "}");
            i = bodyend;
            while (1) {
                int branch = next_statement_token(t, n, i);
                int ifkw = branch;
                int ecopen, ecclose, ebstart, ebend;
                char *econd;
                if (branch < n && (streq(t[branch].v, "Elif") || streq(t[branch].v, "elif"))) {
                    ifkw = branch;
                } else if (branch < n && (streq(t[branch].v, "Else") || streq(t[branch].v, "else"))) {
                    ifkw = next_statement_token(t, n, branch + 1);
                    if (ifkw >= n || !(streq(t[ifkw].v, "If") || streq(t[ifkw].v, "if"))) break;
                } else break;
                ecopen = ifkw + 1;
                while (ecopen < n && !streq(t[ecopen].v, "(")) ecopen++;
                ecclose = matching_paren(t, n, ecopen);
                ebstart = next_statement_token(t, n, ecclose + 1);
                ebend = statement_extent(t, n, ebstart);
                econd = expr_from_tokens(bc->prog, t + ecopen + 1, ecclose - ecopen - 1, &bc->vars, bc->curcls, 0);
                sb_printf(bc->out, " else if (%s) {\n", econd);
                free(econd);
                bc->indent++;
                emit_statement_contents(bc, t, n, ebstart, ebend);
                bc->indent--;
                emit_indent(bc->out, bc->indent);
                sb_add(bc->out, "}");
                i = ebend;
            ...
            {
                int branch = next_statement_token(t, n, i);
                if (branch < n && (streq(t[branch].v, "Else") || streq(t[branch].v, "else"))) {
                    int estart = next_statement_token(t, n, branch + 1);
                    int eend = statement_extent(t, n, estart);
                    sb_add(bc->out, " else {\n");
                    bc->indent++;
                    emit_statement_contents(bc, t, n, estart, eend);
                    bc->indent--;
                    emit_indent(bc->out, bc->indent);
                    sb_add(bc->out, "}\n");
                    i = eend;
        ...
        if (streq(t[i].v, "Do") || streq(t[i].v, "do")) {
            int bodystart = next_statement_token(t, n, i + 1);
            int bodyend = statement_extent(t, n, bodystart);
            int whilekw = next_statement_token(t, n, bodyend);
            if (whilekw < n && (streq(t[whilekw].v, "While") || streq(t[whilekw].v, "while"))) {
                int condopen = whilekw + 1;
                int condclose;
                char *cond;
                while (condopen < n && !streq(t[condopen].v, "(")) condopen++;
                condclose = matching_paren(t, n, condopen);
...
static char *generate_c(Program *p, const char *target_arch) { Str out; sb_init(&out); emit_runtime_prelude(p,&out,target_arch); emit_structs(p,&out); emit_prototypes(p,&out); emit_globals(p,&out); emit_functions(p,&out); return sb_take(&out); }
...
static char *find_root_from_argv0(const char *argv0) {
    char cwd[4096];
    char *dir;
    int depth;
    if (!argv0 || (!strchr(argv0, '/') && !strchr(argv0, '\\'))) {
        ...
        dir = path_dirname(argv0);
    }
    for (depth = 0; depth < 8; ++depth) {
        char *lib = path_join(dir, "lib");
        char *sys = path_join(lib, "Sys");
        char *runtime = path_join(sys, "runtime.c");
        int found = file_exists(runtime);
        free(runtime); free(sys); free(lib);
        if (found) return dir;
        {
            char *parent = path_dirname(dir);
            if (streq(parent, dir)) { free(parent); break; }
            free(dir);
            dir = parent;
        }
    }
...
static char *make_temp_path(const char *prefix, const char *ext)
{
    ...
    const char *base = prefix && *prefix ? prefix : "cmotive_selfhost";
...
static int cmotive_main(int argc, char **argv) { Args a; PPContext pp; Program prog; Parser ps; TokVec tv; char *root, *pptext, *csrc, *out, *tmpc, *cc, *qtmp, *qout, *cmd; int i, rc; Str combined; memset(&a,0,sizeof(a)); parse_args(argc,argv,&a); if(a.version){ printf("CMotive compiler %s\n",CMOTIVE_VERSION); return 0; } cc = getenv("CMOTIVE_CC") ? xstrdup(getenv("CMOTIVE_CC")) : xstrdup("cc"); if(a.print_linker){ puts(getenv("CMOTIVE_LD")?getenv("CMOTIVE_LD"):cc); return 0; } if(a.print_toolchain){ printf("cc=%s\nld=%s\n",cc,getenv("CMOTIVE_LD")?getenv("CMOTIVE_LD"):cc); return 0; } if(a.print_arch){ puts(a.target_arch?a.target_arch:"native"); return 0; } if(a.inputs.n==0 && a.objs.n==0){ fprintf(stderr,"cmotive: no input files\n"); return 2; } root=find_root_from_argv0(argv[0]); memset(&pp,0,sizeof(pp)); pp.root=root; { char *lib=path_join(root,"lib"); sv_push(&pp.include_dirs,lib); free(lib); } for(i=0;i<a.includes.n;i++) sv_push(&pp.include_dirs,a.includes.v[i]); for(i=0;i<a.defs.n;i++){ char *eq=strchr(a.defs.v[i],'='); if(eq){ *eq=0; pp_set_macro(&pp,a.defs.v[i],eq+1); *eq='='; } else pp_set_macro(&pp,a.defs.v[i],"1"); }
 ...
 csrc=generate_c(&prog,a.target_arch?a.target_arch:"native"); if(a.emit_c){ out=a.out?a.out:xstrdup("a.c"); rc=write_file(out,csrc); if(a.debug && a.out) write_debug_files(a.out,&prog,&a); return rc; } out=a.out?a.out:xstrdup(a.compile_only?(a.inputs.n?"a.o":"a.obj"):"a.out"); tmpc=make_temp_path("cmotive_selfhost",".c"); write_file(tmpc,csrc); if(a.keep_c){ char *kc=(char*)xmalloc(strlen(out)+4); sprintf(kc,"%s.c",out); write_file(kc,csrc); free(kc); } qtmp=quote_arg(tmpc); qout=quote_arg(out); { Str cmdb; sb_init(&cmdb); sb_add(&cmdb,cc); sb_add(&cmdb," -I"); { char *inc=path_join(root,"lib/Sys"); char *qi=quote_arg(inc); sb_add(&cmdb,qi); free(qi); free(inc); } sb_add(&cmdb," -I"); { char *qi=quote_arg(root); sb_add(&cmdb,qi); free(qi); } for(i=0;i<a.includes.n;i++){ char *qi=quote_arg(a.includes.v[i]); sb_add(&cmdb," -I"); sb_add(&cmdb,qi); free(qi); } if(a.debug) sb_printf(&cmdb," -g%d",a.debug); if(a.opt&&*a.opt) { sb_ch(&cmdb,' '); sb_ch(&cmdb,'-'); sb_add(&cmdb,a.opt); } if(a.compile_only) sb_add(&cmdb," -c "); else sb_add(&cmdb," "); sb_add(&cmdb,qtmp); if(!a.compile_only){ for(i=0;i<a.objs.n;i++){ char*q=quote_arg(a.objs.v[i]); sb_ch(&cmdb,' '); sb_add(&cmdb,q); free(q); } for(i=0;i<a.libdirs.n;i++){ char*q=quote_arg(a.libdirs.v[i]); sb_add(&cmdb," -L"); sb_add(&cmdb,q); free(q); } for(i=0;i<a.libs.n;i++){ sb_add(&cmdb," -l"); sb_add(&cmdb,a.libs.v[i]); } sb_add(&cmdb," -pthread -lm"); } sb_add(&cmdb," -o "); sb_add(&cmdb,qout); cmd=sb_take(&cmdb); }
