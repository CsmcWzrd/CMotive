...
capture strict-c "$CC_CMD" -std=c11 -Wall -Wextra -Werror=format -I"$ROOT" -I"$ROOT/lib/Sys" "$WORK/smoke-generated.c" -pthread -lm -o "$WORK/strict-c-bin"
check "emitted C compiles in strict C11 mode with format warnings treated as errors" grep -qx 0 "$WORK/strict-c.status"
check_sh "strict-C11 output executes and portable 64-bit integer stream I/O round-trips" '"$WORK/strict-c-bin" && "$CMOTIVE" "$TOOLS/io_int_roundtrip.CMOT" -o "$WORK/io-int-roundtrip" >"$WORK/io-int-roundtrip.compile.log" 2>&1 && printf "922337203685477580\n" | "$WORK/io-int-roundtrip" | grep -qx "922337203685477580"'
...
check_sh "release packaging preserves the QA manifest and complete MIT license" 'rm -rf "$WORK/package" && mkdir -p "$WORK/package" && sh "$ROOT/scripts/package_release.sh" --root "$ROOT" --out "$WORK/package" >/dev/null && archive=$(find "$WORK/package" -name "*.tar.gz" | head -n 1) && test -s "$archive" && tar -tzf "$archive" | grep -Fq "/quality-assurance/manifest.tsv" && tar -xOf "$archive" --wildcards "*/LICENSE" | grep -Fq "Permission is hereby granted, free of charge" && tar -xOf "$archive" --wildcards "*/LICENSE" | grep -Fq "THE SOFTWARE IS PROVIDED \"AS IS\""'
